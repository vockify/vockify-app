...
import 'package:vockify/src/pages/start/widgets/history_term_list.dart';
import 'package:vockify/src/pages/start/widgets/user_term_text_field.dart';
import 'package:vockify/src/pages/term/widgets/definition_chips.dart';
import 'package:vockify/src/pages/term/widgets/spell_check_text.dart';
import 'package:vockify/src/pages/term/widgets/transcription_text.dart';
...
class _StartUserTermFormState extends State<StartUserTermFormWidget> {
          ...
          children: [
            ...
            if (_term.isEmpty)
              HistoryTermListWidget(onTap: (value) {
                ...
                amplitude.logEvent('start_screen_history_term_clicked',
                    eventProperties: {
                      'term': value,
                    });
              }),
  ...
  bool _isTyping() =>
      _onStoppedTyping != null && (_onStoppedTyping?.isActive ?? false);
      ...
      children: <Widget>[
        Expanded(
          child: RawMaterialButton(
            ...
            onPressed: () {
              final term = _term;
              final definition = _selectedDefinitions.isEmpty
                  ? _definitions.first
                  : _selectedDefinitions.join(', ');

              FocusScope.of(context).unfocus();
              Navigator.of(context).push(
                MaterialPageRoute<UserSetSelectPageWidget>(
                  builder: (context) {
                    return UserSetSelectPageWidget(
                      term: term,
  ...
  Future<void> _translate(String text) async {
    ...
    setState(() {
      _term = text;
      _transcription =
          data.length > 0 && data.firstOrNull?.transcription != null
              ? data.first.transcription!
              : '';
      _definitions = data
          .expand((entry) => entry.translations.map((tr) => tr.text))
          .toList();
    });
